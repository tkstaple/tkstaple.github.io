...
;(function($) {
    ...
    ScrollSection.prototype = {
        ...
        findElements: function() {
            ...
            this.extraHeight = this.holderHeight + this.containerHeight;
            this.busy = false;
            this.resizing = false;
            this.page.css({ paddingBottom: this.extraHeight });
        },
        attachEvents: function() {
            ...
            win.on('resize orientationchange', this.onResize);
        },
        scroll: function() {
            var winTop = win.scrollTop();
            if (winTop > this.holderOffset + this.holderHeight + 1) {
                this.page.css({ paddingBottom: '' });
                if (!this.busy) {
                ...
                this.page.css({ paddingBottom: this.extraHeight });
                if (this.busy) {
                    this.container.addClass(this.options.stickyClass);
                    this.busy = false;
                }
            }

            this.makeCallback('fallbackFunc');
            this.resize();
        },
        resize: function() {
            if (!this.resizing) {
        ...
        destroy: function() {
            this.resizing = true;
            this.page.css({ paddingBottom: '' }).removeAttr('style');
            win.off('scroll', this.onScroll);
            win.off('resize orientationchange', this.onResize);
        },
...
;(function($) {
    ...
    StickyPanel.prototype = {
        ...
        initStructure: function() {
            ...
            this.offset = this.getTargetOffset(this.options.extraOffset);
            this.fake = $('<div class="' + this.options.fakeBlockClass + '"></div>');
            this.resizeFlag = false;
            this.isActive = false;
            this.scrollPosition = 0;
            this.scrollPositionPrev = 0;

            if (this.options.fakeBlock){
                this.fake.insertBefore(this.panel).css({ display: 'none', height: this.height });
            }
        },
        attachEvents: function() {
            var self = this;
            ...
            win.on('resize orientationchange', this.onResizeWindow);
        },
        getTargetOffset: function(extraOffset) {
            if (typeof this.options.extraOffset === 'number'){
                extraOffset = this.options.extraOffset;
            } else if (typeof this.options.extraOffset === 'string'){
                extraOffset = $(this.options.extraOffset).offset().top + $(this.options.extraOffset).outerHeight();
            }
            return extraOffset;
        },
        scrollHandler: function() {
            ...
            if (winTop > this.offset){
                if (!this.isActive){
                    this.panel.addClass(this.options.slickyClass);
                    if (this.options.fakeBlock) this.fake.show();
                    this.isActive = true;
                }
            } else {
                if (this.isActive){
                    this.panel.removeClass(this.options.slickyClass);
                    if (this.options.fakeBlock) this.fake.hide();
                    this.isActive = false;
                }
            }

            if (this.options.scrollClasses){
                this.scrollPosition = winTop;
                if (this.scrollPosition > this.scrollPositionPrev){
                    this.panel.addClass(this.options.scrollBottomClass);
        ...
        windowResize: function() {
            if (this.options.fakeBlock){
                this.height = this.panel.outerHeight();
                this.offset = this.getTargetOffset(this.options.extraOffset);
                this.fake.css({ height: this.height });
            } else {
                this.offset = this.getTargetOffset(this.options.extraOffset);
            }
            this.scrollHandler();
        ...
        destroy: function() {
            console.log(1)
            if (this.options.fakeBlock) this.fake.remove();
            this.resizeFlag = true;
            this.panel.removeClass(this.options.slickyClass).removeClass(this.options.scrollTopClass).removeClass(this.options.scrollBottomClass);
            win.off('scroll', this.onScrollHandler);
            win.off('resize', this.onResizeWindow);
        }
    };

    // jQuery plugin interface
    $.fn.stickyPanel = function(opt) {
        return this.each(function() {
            jQuery(this).data('StickyPanel', new StickyPanel($.extend(opt, { panel: this })));
        });
    };
}(jQuery));
...
(function (window, $, undefined) {
    ...
    $.infinitescroll.defaults = {
        loading: {
            finished: undefined,
            finishedMsg: "<a href='#' class='btn btn-primary js-load-more'>No more projects</a>",
            ...
            msg: null,
            msgText: "<a href='#' class='btn btn-primary js-load-more'>Loading more projects</a>",
            selector: null,
            speed: 'fast',
            start: undefined
        },
